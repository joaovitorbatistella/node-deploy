...
let ListProvidersService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  constructor(usersRepository, caheProvider) {
  ...
  async execute({
    ...
    let users = await this.caheProvider.recover(`providers-list:${user_id}`);

    if (!users) {
      users = await this.usersRepository.findAllProviders({
        except_user_id: user_id
      });
      await this.caheProvider.save(`providers-list:${user_id}`, (0, _classTransformer.classToClass)(users));
    }
