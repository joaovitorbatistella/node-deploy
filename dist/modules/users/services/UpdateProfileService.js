...
let UpdateProfile = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (password && !old_password) {
      throw new _AppError.default('You need to inform old password to set a new password.');
    }

    if (password && old_password) {
      const ckeckOldPassword = await this.hashProvider.compareHash(old_password, user.password);
